...
<!-- --- -->
# SymForce 란?
- 최근 RSS 2022 에서 [SymForce](https://symforce.org/){:target="_blank"} 라는 framework 이 공개되었다. Skydio 가 공개하였다. 내부적으로 5년동안 개발하고 실제로 사용해온 것이라고 한다.
...
- [SymForce 논문](https://arxiv.org/abs/2204.07889){:target="_blank"}을 보면 robotics (특히 드론 등)와 같이 빠른 계산이 실시간으로 요구될 때에는, (Ceres 같은) 계산그래프 기반보다 __symbolic 기반__ 의 미분이 더 좋다고 주장하고 있다. 원래 Robotics 에서 Ceres 가 많이 쓰이고 있어서 새로운 solver 의 필요성을 못 느끼고 있었는데, 이 논문의 말을 들어보면 왠지 설득이 된다.
- 한 번 공부할겸 써보았다.
